<!-- Londy details -->
<div class="5grid 5grid-container hide more londy">
	<div class="row">
		<div class="12u">
			<article class="box box-style2">
				<h3>Todo App for teams</h3>

				<p>This web app is a small TODO application for managing projects and tasks in a working team.</p>
				<p>This is implemented using Ruby on Rails for the back end and Spine.js for the front end.</p>
				<p>Watch the demo video to have a look at how it works.</p>
				<a class="mobile_only button button-small" href="http://www.youtube.com/watch?v=OzVLr49Kf8c" class="button button-small">Watch it on Youtube</a>
				<div class="desktop_only 1000_px_only">
					<iframe  width="420" height="315" id="youtube_londy"  frameborder="0" allowfullscreen></iframe>

				</div>

			</article>
		</div>
	</div>
</div>